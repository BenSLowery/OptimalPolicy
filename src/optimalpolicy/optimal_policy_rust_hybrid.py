...
class optimal_policy():
    ...
    def _pre_processing(self):
        ...
        for idx, state in enumerate(self.state_space):
            print(idx, end='\r')
            self.G_w[state] = rust_helpers.expectation_warehouse(tuple(state), self.s1_demand[1][0],self.s2_demand[1][0],self.h_w,self.p)
            self.G_s[state] = rust_helpers.expectation_store(tuple(state), self.s1_demand[1][0],self.s2_demand[1][0],self.h_w,self.c_u, self.c_p, self.p)

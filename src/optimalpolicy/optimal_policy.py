...
# SLOW, and you shouldn't use   #
# Used a demonstrative purposes # 
# Also incorrect calculation of #
# the store costs               #
#################################
...
class optimal_policy():
    ...
    def _pre_processing(self):
        ...
        self.state_space = [x for x in itertools.product([i for i in range(self.max_wh) for j in range(self.l_w)], [i for i in range(self.max_s1)], [i for i in range(self.max_s2)])]
        ...
        for idx, state in enumerate(self.state_space):
            print(idx, end='\r')
            self.G_w[state] = self.immediate_cost_warehouse(state)
            self.G_s[state] = self.immediate_cost_store(state,self.s1_d_pmf,self.s2_d_pmf)
    ...
    def immediate_cost_warehouse(self, state):
        ...
        for d_1,d_1_pmf in self.s1_d_pmf:
            for d_2,d_2_pmf in self.s2_d_pmf:
                ...
                max_beta_s2 = min(max(d_2-state[2],0),state[0])

                beta_1_pmf = [(v,sp.binom.pmf(v,max_beta_s1,self.p)) for v in range(max_beta_s1+1)]
               
                for b1,b1_pmf in beta_1_pmf:
                    max_beta_s2 = min(max(d_2-state[2],0),state[0]-b1)
                    beta_2_pmf = [(v,sp.binom.pmf(v,max_beta_s2,self.p)) for v in range(max_beta_s2+1)]
                    for b2,b2_pmf in beta_2_pmf:
                        exp += d_1_pmf*d_2_pmf*b1_pmf*b2_pmf*self.h_w*(state[0]-(b1+b2))
    ...
    def immediate_cost_store(self, state, store_1_demand,store_2_demand):
        ...
        for d_1,d_1_pmf in store_1_demand:
            ...
            if max_beta > 0:
                ...
                beta_pmf = [(v,sp.binom.pmf(v,max_beta,self.p)) for v in range(max_beta)]
        ...
        for d_2,d_2_pmf in store_2_demand:
            ...
            if max_beta > 0:
                # Find the beta, beta_pmf combination (we could precalculate this?)
                beta_pmf = [(v,sp.binom.pmf(v,max_beta,self.p)) for v in range(max_beta+1)]
                for b, b_pmf in beta_pmf:
                    # Add the cost of partial lost sales 
                    # (here we have the cost of choosing partial lost sales minus any 
                    # potential lost sales we incurred intially before the partial lost sales)
    ...
    def future_cost(self, post_action_state, wh_order, st_1_order, st_2_order,V_t_plus_1):
        exp = 0
            ...
            for d_2,d_2_pmf in self.s2_d_pmf:
                ...
                for b1,b1_pmf in beta_1_pmf:
                    ...
                    for b2,b2_pmf in beta_2_pmf:
                        ...
                        inner_exp += b1_pmf*b2_pmf*V_t_plus_1[(wh_next_state,st_1_next_state,st_2_next_state)]
    ...
    def generate_action_space(self, wh, st_1_pre_ts, st_2_pre_ts):
        ...
        ts_options = [(0,0)] + [(i,0) for i in range(1,min(st_1_pre_ts+1,self.max_s2-st_2_pre_ts))] + [(0,i) for i in range(1,min(st_2_pre_ts+1,self.max_s1-st_1_pre_ts))]
        valid_actions = []
        for ts_s1, ts_s2 in ts_options:
            ...
            for order_st_1 in range(max(min(self.max_s1_action-st_1,wh-st_1),1)):
                for order_st_2 in range(max(min(self.max_s2_action-st_2,wh-st_2),1)):
                    if order_st_1+order_st_2 <= wh:
                        for wh_order in range(self.max_wh_action-wh):
                            valid_actions.append((wh_order, order_st_1,order_st_2,ts_s1,ts_s2))
    ...
    def value_function(self, state, V_t_plus_1):
        ...
        for (wh_order, st_1_order, st_2_order, st_1_ts, st_2_ts) in actions:
            post_ts_order_state = (wh-st_1_order-st_2_order, st_1-st_1_ts+st_2_ts, st_2-st_2_ts+st_1_ts)
            
            # Immediate cost (warehouse_cost + transhipment cost + store cost)
            im_cost = self.c_ts*(st_1_ts+st_2_ts)+self.G_w[post_ts_order_state]+self.G_s[post_ts_order_state]
            
            # Future cost
            fut_cost = self.gamma*self.future_cost(post_ts_order_state, wh_order, st_1_order, st_2_order, V_t_plus_1)
            total_cost_all_actions[(wh_order, st_1_order, st_2_order, st_1_ts, st_2_ts)] = im_cost+fut_cost

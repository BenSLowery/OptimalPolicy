...
for c_h in [1, 2, 3]:
    for dfw in [0.2, 0.5, 0.8]:
        ...
        for transhipment_policy in ["E", "T", "N"]:
            print(
                "Calculating for c_h={}, dfw={}, policy={}".format(
                    c_h, dfw, "TIE" if transhipment_policy == "T" else "No Transhipment"
                )
            )
            for wh in [i for i in range(7, 12)]:
                for alpha in [0.7, 0.75, 0.8, 0.85, 0.9, 0.95]:
                    ...
                    pol_eval_res[
                        "({},{}, {})".format(wh, alpha, transhipment_policy)
                    ] = (
                        min(val_bs, key=val_bs.get),
                        val_bs[min(val_bs, key=val_bs.get)],
                    )
        pickle.dump(
            pol_eval_res,
            open("test/validate/best_results_{}_{}.pkl".format(c_h, dfw), "wb"),
        )
